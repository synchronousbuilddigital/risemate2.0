...
const ContactCard = ({ title, email, phone, logo, refLabel, textColor, bgLightColor, bgGlowColor, link }) => (
    <motion.div
        ...
        className={`relative p-5 md:p-10 ${bgLightColor} border border-dark/5 rounded-[20px] md:rounded-[40px] hover:border-dark/10 hover:shadow-[0_45px_90px_-20px_rgba(0,35,102,0.08)] transition-all duration-700 group overflow-hidden ${link ? 'cursor-pointer' : ''}`}
    >
        ...
        <div className="relative z-10">
            <div className="flex items-start mb-5 md:mb-10">
                <div className="w-10 h-10 md:w-14 md:h-14 bg-white shadow-xl rounded-xl md:rounded-2xl flex items-center justify-center group-hover:-translate-y-1 transition-all duration-500 overflow-hidden border border-dark/5">
                    {logo && <Image src={logo} alt={title} width={40} height={40} className="object-contain" unoptimized={true} />}
                </div>
            </div>
            <div className="mb-4 md:mb-6">
                <p className="text-[7px] md:text-[10px] font-black uppercase tracking-[0.2em] md:tracking-[0.45em] text-dark/30 mb-1 md:mb-2">{refLabel}</p>
                <h3 className={`text-base md:text-2xl font-black ${textColor} tracking-tighter leading-none`}>{title}</h3>
            </div>
            <div className="space-y-2 md:space-y-3">
                <a href={`mailto:${email}`} className="block text-[9px] sm:text-xs md:text-sm font-bold text-dark/70 hover:text-dark transition-colors truncate">{email}</a>
                {phone && (
                    <p className={`text-[8px] md:text-[10px] font-black uppercase tracking-wider md:tracking-widest ${textColor} flex items-center gap-1 md:gap-2 leading-none`}>
    ...
    return (
        <div className="bg-[#FAF9F6] min-h-screen selection:bg-[#002366] selection:text-white overflow-x-hidden">
            ...
            <main className="pt-16 md:pt-24">
                {/* HERO */}
                <section className="relative pt-6 pb-12 md:py-40 px-6 md:px-24 overflow-hidden">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                            ...
                            >
                                <div className="flex items-center gap-4 mb-6 md:mb-12">
                                    ...
                                    <span className="text-xs font-black uppercase tracking-[0.3em] md:tracking-[0.6em] text-[#002366]">Initialize Connection</span>
                                </div>
                ...
                {/* HUB INFRASTRUCTURE */}
                <section className="py-10 md:py-32 bg-[#f8faff] px-6 md:px-24 relative overflow-hidden">
                    <div className="max-w-screen-2xl mx-auto relative z-10">
                        <motion.div
                            variants={fadeInUp}
                            initial="initial"
                            whileInView="whileInView"
                            className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-6 md:mb-32 w-full text-left"
                        >
                            <div className="max-w-xl text-left">
                                <span className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.3em] md:tracking-[0.6em] text-[#002366] mb-3 md:mb-6 block text-left">Physical Infrastructure</span>
                                <h2 className="text-4xl md:text-8xl font-black text-dark tracking-tighter leading-[1.1] md:leading-none text-left">Global <br className="hidden md:block" /> Hubs.</h2>
                            </div>
                            <p className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.2em] md:tracking-[0.4em] text-dark/30 italic text-left md:text-right mt-2 md:mt-0">Operational Ref: HQ_SYNC_2026</p>
                        </motion.div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-12">
                            {[
                                ...
                                <motion.div
                                    ...
                                    initial="initial"
                                    whileInView="whileInView"
                                    className="group relative p-6 md:p-16 bg-white rounded-[24px] md:rounded-[64px] border border-[#002366]/5 overflow-hidden shadow-[0_20px_40px_rgba(0,0,0,0.02)]"
                                >
                                    {/* Hide watermark on mobile/tablet to prevent overlapping with text */}
                                    <div className="hidden lg:block absolute top-0 right-0 p-16 text-[150px] font-black text-[#002366]/[0.02] leading-none pointer-events-none group-hover:text-[#002366]/[0.05] transition-all duration-1000">
                                        {hub.code}
                                    </div>
                                    <div className="relative z-10">
                                        <div className="flex items-center gap-4 mb-3 md:mb-10">
                                            <span className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.2em] md:tracking-[0.6em] text-[#002366] bg-[#002366]/5 px-3 py-1.5 md:px-5 md:py-2.5 rounded-full">{hub.type}</span>
                                        </div>
                                        <h4 className="text-2xl md:text-4xl font-black text-dark mb-1 md:mb-2 tracking-tighter leading-none">{hub.label}</h4>
                                        <p className="text-lg md:text-2xl font-black text-[#002366] mb-3 md:mb-10">{hub.city}</p>
                                        <div className="h-[1px] w-full bg-dark/5 mb-3 md:mb-10" />
                                        <p className="text-sm md:text-2xl text-dark/40 font-secondary leading-tight max-w-sm mb-4 md:mb-16">
                                            {hub.address}
                                        </p>
                                        <div className="flex items-center gap-2 md:gap-4">
                                            <div className="w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-[#002366]"></div>
                                            <p className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.2em] md:tracking-[0.4em] text-dark/20">
                                                {hub.ref}
                                            </p>
                                        </div>
                                    </div>
                ...
                {/* MANDATE PORTAL */}
                <section className="py-16 md:py-40 bg-[#001233] text-white relative overflow-hidden">
                    ...
                    <div className="container-wide relative z-10">
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 md:gap-32 items-center">
                            <motion.div
                                ...
                                whileInView="whileInView"
                            >
                                <span className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.3em] md:tracking-[1em] text-[#002366] bg-white px-4 py-2 md:px-5 md:py-2.5 rounded-full mb-6 md:mb-12 inline-block">Secure Protocol</span>
                                <h2 className="text-4xl md:text-8xl font-black mb-6 md:mb-12 tracking-tighter leading-[0.9]">Ready to <br /><span className="text-white/40 italic font-serif font-normal">Initialize?</span></h2>
                                <p className="text-base md:text-2xl text-white/40 font-secondary leading-tight max-w-lg mb-8 md:mb-16 italic">
                            ...
                            <motion.div
                                ...
                                initial="initial"
                                whileInView="whileInView"
                                className="bg-white/[0.02] backdrop-blur-3xl border border-white/10 p-6 md:p-20 rounded-[32px] md:rounded-[80px] shadow-[0_80px_160px_-40px_rgba(0,0,0,0.5)]"
                            >
                                <form className="space-y-6 md:space-y-12">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-12">
                                        <div className="space-y-2 md:space-y-4">
                                            <label className="text-[9px] font-black uppercase tracking-[0.3em] md:tracking-[0.5em] text-white/30 block ml-2">Full Identity</label>
                                            <input type="text" className="w-full bg-white/[0.03] border-b border-white/10 rounded-none px-2 py-3 md:py-5 text-white focus:border-white outline-none transition-all placeholder:text-white/10" placeholder="LEAD NAME" />
                                        </div>
                                        <div className="space-y-2 md:space-y-4">
                                            <label className="text-[9px] font-black uppercase tracking-[0.3em] md:tracking-[0.5em] text-white/30 block ml-2">Corporate Email</label>
                                            <input type="email" className="w-full bg-white/[0.03] border-b border-white/10 rounded-none px-2 py-3 md:py-5 text-white focus:border-white outline-none transition-all placeholder:text-white/10" placeholder="EXEC@ENTITY.COM" />
                                        </div>
                                    </div>
                                    <div className="space-y-2 md:space-y-4">
                                        <label className="text-[9px] font-black uppercase tracking-[0.3em] md:tracking-[0.5em] text-white/30 block ml-2">Mandate Specification</label>
                                        <textarea rows="4" className="w-full bg-white/[0.03] border-b border-white/10 rounded-none px-2 py-3 md:py-5 text-white focus:border-white outline-none transition-all placeholder:text-white/10" placeholder="Strategic objectives and vertical focus..."></textarea>
                                    </div>

                                    <div className="pt-4 md:pt-8">
                                        <button className="group relative w-full h-16 md:h-24 overflow-hidden rounded-full transition-all duration-700">
                                            ...
                                            <span className="relative z-10 text-[10px] md:text-[11px] font-black uppercase tracking-[0.3em] md:tracking-[0.8em] text-dark group-hover:text-white transition-colors duration-700">
                                                Transmit Mandate
                                            </span>
                                        </button>
                                    </div>
